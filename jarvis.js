...
$(function() {
    if (annyang)
    {
        var commands =
        {
            ...
            'stop classical': function() {stop_classical();},
...
{
    notify();
    tab_classical = window.open('http://www.radioline.co/listen-to-kpbs-2-classical-san-diego#radios/kpbs-2-classical-san-diego','Classical San Diego','width=600,height=100');
}

function stop_classical()
{
    notify();
    if (tab_classical)
        tab_classical.close();
...
function take_note(note)
{
    ...
    formObj["data"] = note + "\n" + clipText;
    //formObj["latitude"] = coords.latitude;
    //formObj["longitude"] = coords.longitude;
    formObj["action"] = "take";
    console.log(formObj);
    $.ajax({
            url: "./note.py",
